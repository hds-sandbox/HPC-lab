...
To run the workflow in Snakemake, you can either 1) request Snakemake to generate a specific file (using the exact filename as defined in the rule's output) or 2) specify the name of a rule or task. The only mandatory flag in Snakemake is the number of cores (`-c`) or jobs (`-j`), which indicates the number of processes that can run in parallel.

```{.bash}
# Run the workflow: Call either the last rule or the last output, which will trigger the same set of rules.
snakemake -c2 my-plot.png 
# or
snakemake -c2 plotl
```
