...
4. Add folders to access while in this job (select your own drive!)
    ...
    - You need to create a folder called `pipesOut` in your own drive, where you will save any output from the exercises. You won’t have writing permissions on our shared drive.
...
conda env create -f pipesOut/hpclab-env.yml 
```
...
`conda create --prefix /work/pipesOut/envs/<test-env>` 
`conda create --name <my-env>`
...
3. UCloud: `conda activate /work/pipesOut/envs/<test-env>` or genomedk: `conda activate <myenv>`
4. The executable is located at `/usr/bin/python` in the system
5. conda install bioconda::snakemake
6. Snakemake is working!
7. Yes, now it is in the bin of our env: `/work/pipesOut/envs/<test-env>/bin/python` or `~/miniforge3/envs/test/bin/python`
...
```{.bash}
# 1. UCloud 
conda env create --file /work/HPCLab_workshop/data/test-env.yml --prefix /work/pipesOut/envs/test-env
# genomedk / local 
conda create --file test-env.yml
# 2. 
conda deactivate; UCloud: `conda activate /work/pipesOut/envs/test-env` 
# 3.
conda list 
# 4. 
