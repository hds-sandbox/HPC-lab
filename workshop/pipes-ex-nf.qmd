...
You will be using the `Nextflow` app, version `24.10.5`. First, mount the two drives below, and ask for 2 CPUs so we can run things in parallel: 
...
- Use the `conda` profile to manage the software environment for your pipeline. Among other options, you can include the conda environment within a process itself or specify it from the command line using the `--with-conda` flag, or use a config file to define it. We recommend to specify it via the command line (`--with-conda`), and incorporate the environment within the process in the next exercise once the pipeline is working.
...
For now, lets just define, the resources we want this pipeline to use:
...
Make sure to go through the hint first, as it will provide useful tips and serve as inspiration. There isn't just one solution, so the hint can guide you toward different approaches to solving the problem.
...
It is highly recommended to click on the hint below. 
...
process PREPROCESS {
    ...
    output:
        path "samples_${species}_cleaned2.tsv"

    script:
    """
    python /work/AlbaRefoyoMartínez#0753/hpclab/scripts/preprocess.py $input_samples samples_${species}_cleaned2.tsv
    """
}

workflow {
    ch_species = Channel.of('EB', '1kgp')
    ch_input   = ch_species.map { sp -> file("${params.input_dir}/samples_${sp}.tsv") }

    PREPROCESS(ch_species, ch_input)
}
```
:::
...
The `filter_year.py` script needs a value for flag cutoff. It's better to define cutoff as a parameter (e.g., `params.cutoff = 2000`), which can then be modified using the config file or the command line. This approach provides flexibility to adjust the value without changing the pipeline code.
