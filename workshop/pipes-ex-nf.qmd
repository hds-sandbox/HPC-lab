...
- **Directory setup**: Assuming you've followed the setup instructions and launched the Nextflow app by selecting `Open terminal`, create a new subdirectory inside `hpcPipes`—for example,`nf-pipes`, to store your pipeline and resulting outputs. Make sure it's set as your working directory (e.g. `cd hpcPipes/nf-pipes`). 
...
rule preprocess:
  input:
    "/work/HPCLab_workshop/pipes/data/samples_EB.tsv"
  output:
...
- Set the `HOME` environment variable in your terminal to your project directory (e.g., `export HOME=/work/hpcPipes/nf-pipes`). 
