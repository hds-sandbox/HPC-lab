...
- `pipesOut`: from your personal drive and save your results/files here.  
...
Finally, navigate to your wd, *pipesOut*!

## <img src="images/genomedk.png" height="50"> 

1. Create a conda env with nextflow and activate the environment 
```.bash
conda create --name nf-env bioconda::nextflow
source activate nf-env
```

You can also follow the [instructions](https://docs.seqera.io/nextflow/install#self-install) to self-install the package (which is usually recommended). 

2. Navigate to the directory you created in your workspace to save all output (`pipesOut`). 
...
4. `exit` the interactive job when you are done with the exercise! 


## <img src="images/computer.png" height="50"> 

Download the data. The solutions and suggestions below use full paths assuming you are working on UCloud and have followed the setup instructions. If you are running things locally, you will need to adjust these paths accordingly (e.g., `/work/` will not be valid in your computer).
...
- **Directory setup**: Assuming you've followed the setup instructions, create a new subdirectory in your WD—for example,`nf-pipes`, to store your pipeline and resulting outputs. Make sure it's set as your working directory (e.g. `cd nf-pipes`). 
...
In this task, you’ll develop a simple Nextflow pipeline based on the first version of the `step1_wf.smk` Snakemake file you worked with (Day 2, Part 3.I). This pipeline preprocesses a sample metadata file using a Python script.
...
- Set the `HOME` environment variable in your terminal to your project directory (e.g., on UCloud `export HOME=/work/pipesOut/nf-pipes`). 
...
- Use a hardcoded path to the Python script for now: `/PATH/TO/scripts/preprocess.py`
...
- Take a look inside `work` directory and explore some of the files (e.g. `.command.sh` and `.command.log` using `tree -a`). Did Nextflow execute the command you expected? 
...
- 'copyNoFollow': Copies the output files into the publish directory without following symlinks i.e. copies the links themselves.
...
# on UCloud, path is different for genomedk/local setup
...
// Pipeline parameters
params.input_dir = "work/shared/pipes/data"
...
process PREPROCESS {
    ...
    script:
    """
    python /PATH/TO/pipesOut/scripts/preprocess.py $input_samples samples_EB_cleaned.tsv
    """
}
...
rule preprocess:
    input:
        "/PATH/TO/data/samples_{dataset}.tsv"
    output:
...
This solution has been run on UCloud, so /work/HPCLab_workshop/miniconda3/envs/snakemake would have to be changed to  /path/to/an/existing/env/directory. 
...
rule preprocess:
    input:
        "/PATH/TO/HPCLab_workshop/data/samples_{dataset}.tsv"
    output:
        tsv="results/samples_{dataset}_cleaned.tsv"
    shell:
        """
