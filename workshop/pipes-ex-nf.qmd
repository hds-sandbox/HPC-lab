...
- `hpcPipes`: from your personal drive and save your results/files here.  
...
Finally, navigate to your wd, *hpcPipes*!
...
2. Navigate to the directory you created in your workspace to save all output. 

3. Remember to get an interactive session in SLURM and do not run any nextflow command on the login node. 
```.bash
srun --account DeiC-KU-L65 -t 00:00:30 --pty bash
```
...
- **Directory setup**: Assuming you've followed the setup instructions, create a new subdirectory inside `hpcPipes`—for example,`nf-pipes`, to store your pipeline and resulting outputs. Make sure it's set as your working directory (e.g. `cd hpcPipes/nf-pipes`). 
...
- Set the `HOME` environment variable in your terminal to your project directory (e.g., on UCloud `export HOME=/work/hpcPipes/nf-pipes`). 
...
- Use a hardcoded path to the Python script for now: `/PATH/TO/hpcPipes/scripts/preprocess.py`
...
process PREPROCESS {
    ...
    script:
    """
    python /PATH/TO/hpcPipes/scripts/preprocess.py $input_samples samples_EB_cleaned.tsv
    """
}
