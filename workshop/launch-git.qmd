...
Finally, to streamline pushing changes...
  ...
  7. Create a new initialization bash script or modify an existing one to include this code (*if* you have one). Here, we named the file `setup.sh`: 
...
Remember to save this file in your personal drive (`/work/myNameXXX/hpcLaunch`). Once you have saved the file, click on `stop the application`. 
...
Submit a new job on UCloud (e.g. through the `Terminal` app). Do not forget to mount the hpcLaunch folder on your personal drive and use the initialization bash script you just created (`setup.sh`). Once, the job is running, change directory to day1, and clone the repository you created. 
...
You are done for now! Remember to stop the job when you are done with this exercise ('Stop Application').
