...
You will be working and running the workflow from `hpclab`, so make sure it's set as your working directory. Launch Coder application (by selecting `Open interface`). 

![My Coder app](./images/smk_coder.png)

- Directory Setup: Inside `hpclab`, create a subdirectory called `scripts`. 
- File creation: open a new file named `step1_wf.smk` and include the following two lines of code:
...
:::{.callout-hint}
# Details about previous dataset
...
Here, for each dataset in the Python list `DATASETS`, the file `{dataset}/file.A.gz` is requested. Snakemake, will recognise that these files can be created my multiple applications (executions) of the rule `file_conversion`. 

It's time for you to define the target rule `rule all` within the Snakefile and utilize some helper functions. Instead of using a Python list comprehension to specify input files, Snakemake provides the helpful `expand()` function, use it! 

- Declare the variable DATASETS at the top 
- Define `rule all` and use the `expand()` function to specify the input files
...
Let's assume that `step1_wf.smk` includes all the necessary steps for preprocessing our data. Now, let's add other components to the workflow to further process the data. The aim of the next exercise is to modularized our workflow and decorate the workflow using some advanced functionality. 
...
1. File creation: open a new file named `step2_wf.smk` and define the external workflow `step1_wf.smk` as a module, making all rules available by explicitly importing them and rename them as `step1_*`.
2. Remove `results/samples_EB_cleaned.tsv` to test the newly implemented feature.
...
Let's add a new rule named `filter_year` that replicates the behavior of the bash script below:

```{.bash filename="filter_year.sh"}
INFILE="results/samples_EB_cleaned.tsv"
OUTFILE="results/samples_EV_filtered.tsv"

# python script filters the metadata based on the year the samples were collected 
python scripts/filter_year.py -i $INFILE -o $OUTFILE -y 2000
```
...
**How many samples got filtered out?** Compared the `*_cleaned.tsv` vs the `*_filtered.tsv`. `r fitb(answer='2')`
...
Modify your Snakemake workflow to include a YAML configuration file and use it within the `filter_year` rule. 
