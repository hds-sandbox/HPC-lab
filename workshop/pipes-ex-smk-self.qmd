...
In all cases, for each dataset in the Python list `DATASETS`, the file `{dataset}/file.A.gz` is requested. Snakemake, will recognise that these files can be created my multiple applications (executions) of the rule `file_conversion`. 
...
1. File creation: open a new file named `step2_wf.smk` and define the external workflow `step1_wf.smk` as a module, making all rules available by explicitly importing them and rename them as `step1_*`. Hint: example above. 
