...
- `pipesOut`: from your personal drive. Save your results/files here.  
- `shared/hpclab-workshop`: contains input files and scripts. You can read-only access from this directory (no write permissions). 
    - `/work/HPCLab_workshop/pipes/data`: data input files 
...
Launch the Coder application (by selecting `Open interface`). 
...
2. The necessary files can be found in `pipes`. The snakemake file is located at: `rules/process_1kgp.smk`.
...
Download the `Snakefile` and data required for this exercise using the links below if you are running things locally. Activate your environment where you have the software install. 
...
Move the files into the data folder you created for the previous exercise. 
...
You will be working and running the workflow from `pipesOut`, so make sure it's set as your working directory.

##### Preparation step 

- **Directory setup**: Inside `pipesOut`, create two subdirectories, `scripts` and `rules`. 
...
- Define inputs and outputs:
  - Input: metadata file ('data/samples_EB.tsv')
...
INFILE="data/samples_EB.tsv"
OUTPUT="results_eb/samples_EB_cleaned.tsv"
...
- Input: "pipes/data/samples_1kgp.tsv" (UCloud, `/work/HPCLab_workshop/`)
...
Filepath for input tweaked for UCloud. You will need to adapt the path for GenomeDK. 

```{.python filename="step1_wf.smk"}
rule preprocess:
  input:
    "/work/HPCLab_workshop/pipes/data/samples_{dataset}.tsv"
  output:
...
Filepath for input in rule preprocess tweaked for UCloud. You will need to adapt the path for GenomeDK. 
...
rule step1_preprocess:
    input: /PATH/TO/data/samples_EB.tsv
    ...
    resources: tmpdir=<TBD>
        ...
        python scripts/preprocess.py </PATH/TO/data/samples_EB.tsv> results/samples_EB_cleaned.tsv
