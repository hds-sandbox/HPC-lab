...
**1. python myscript.py** `r torf(FALSE)`
**2. make** `r torf(TRUE)`
**3. bash create_dirs.sh (you created the file)** `r torf(TRUE)`
**4. cookiecutter mytemplate** `r torf(TRUE)`
**5. tar -xvf R-4.6.0.tar.gz** `r torf(TRUE)`
**6. nextflow run pipeline.nf** `r torf(FALSE)`
**7. sbatch mypipeline.sh** `r torf(TRUE)`
**8. nano config.yaml** `r torf(TRUE)`
**9. git clone https://github.com/user/project.git** `r torf(TRUE)`
**10. cp -r 5TB_dataset backup/** `r torf(FALSE)`
**11. rsync -av data/ project_backup/** `r torf(TRUE)`
**12. top** `r torf(TRUE)`
...
**6. I must backup all generated files, including intermediate files, to make sure the analysis are reproducible.** `r torf(FALSE)`

**7. I should not fill up my home folder with data. ** `r torf(TRUE)`

**8. Virtual environments keeps project-specific software and their dependencies separated - without interfering with each other. ** `r torf(TRUE)`

**9. I must always run the analysis/pipeline on a small subset of the data to estimate CPU/RAM resources.** `r torf(TRUE)`

**10. I should run a community-curated pipeline for the first time on all my samples** `r torf(FALSE)`
...
Imagine you need to transfer 11,570 files to an HPC cluster. Which of the following approaches would likely be the most efficient? `r fitb(answer='E')`
