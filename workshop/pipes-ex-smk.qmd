...
- `hpcPipes/smk`: save your results/files here (create the new subdir).  
- `hpclab-workshop`: contains input files and scripts. You can read-only access from this directory (no write permissions). The snakemake file is located at: `/work/HPCLab_workshop/rules/process_1kgp.smk`. 
- Additional parameters - Initialization. Use a setup.sh script that contains snakemake software. You can also use ours, `shared/hpclab-workshop/setup.sh`
...
Use the conda or pixi environment you have already created to run the exercises. Remember to get an interactive session in SLURM and do not run any snakemake command on the login node. 

The necessary files can be found in `smk-exercises`. The snakemake file is located at: `rules/process_1kgp.smk`.
