...
- `hpclab-workshop`: this contains the input files, You can read-only access from this directory (no write permissions). 

Next, deactivate `hpclab-env` and activate `snakemake` environment. 
...
8. Copy the snakefile (`process_1kgp.smk`) to your working directory so that you can edit it! Then, open the file and remove lines 13-15. How else can you run the workflow but to generate instead `all_male.txt` using only the command-line? 
...
cp /work/HPCLab_workshop/rules/process_1kgp.smk .
