...
9. Let's add a new rule that concatenates the two files you have generated (`all_female.txt` and `all_male.txt`) and save it into `concatenated.txt`. Remember, all files should be saved into the `results` subdir. Hint: `cat file1.txt file2.txt > output.txt`
...
rule concat: 
   ...
   shell:
      "cat {input} > {output}"
# 10. Run again 
snakemake -s process_1kgp.smk -c1 results/concatenated.txt 
