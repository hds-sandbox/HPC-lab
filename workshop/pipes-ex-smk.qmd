...
2. Remember to get an interactive session in SLURM and do not run any snakemake command on the login node. 

```.bash
srun --account DeiC-KU-L65 -t 00:00:30 --pty bash
```

3. The necessary files can be found in `smk-exercises`. The snakemake file is located at: `rules/process_1kgp.smk`.
