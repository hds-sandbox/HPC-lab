...
:::{.callout-warning}
When mounting the data, ensure that the path you provide actually exists. If you encounter an error indicating that the .bfile does not exist, it likely means the data was not mounted correctly.

Tip for Windows users: The correct path might be `${PWD}\toy\toy` — double-check that the folder structure are accurate.
:::
