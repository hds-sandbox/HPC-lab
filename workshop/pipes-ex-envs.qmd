...
- **Bonus.1. Numpy version** `r fitb(answer='2.1.1')`
- **Bonus.2. Full ID version** `r fitb(answer='48606ab22bfdb0e9d7ec4ed5eef984b873b7796d')`
...
::: {.webex-check .callout-exercise}
# Part 3 - Adapt a colleague's Conda environment to fit your needs.

If you are running this exercise locally, download the environment yaml file first: 

<div style="display: flex; gap: 10px;">
  <a href="./data/test-env.yml" download="test-env.ymlp">
    <button class="btn small-button">Download environment file</button>
  </a>
</div>

Create a Conda environment using the provided 'test-env.yml' file. Please, use the flag `--prefix` for this purpose. After the environment is set up:

- Verify the installed packages (you will also see other dependencies installed)
- Remove the 'cookiecutter' package
- Update Python to the latest compatible version. Which version is that? `r fitb(answer='3.7.16')`
- Try to install the latest numpy version using `conda install numpy=2.1`, could you successfully install it? `r torf(FALSE)`  
...
```{.bash}
conda env create --file test-env.yml --prefix /work/AlbaRefoyoMartínez#0753/envs/test-env
conda activate /work/AlbaRefoyoMartínez#0753/envs/test-env 
conda list 
...
Refer to the official [Conda cheat sheet](https://docs.docker.com/get-started/docker_cheatsheet.pdf) for guidance! 



::: {.webex-check .callout-exercise}
# Part 4 - Running a docker image

In this exercise, we will utilize the `fastmixture` Docker image available on [DockerHUb](https://hub.docker.com/r/albarema/fastmixture). The aim is to **understand how to run containerized software**. In our example, we have selected a fast option and a toy dataset to facilitate learning. Focus on executing the commands, ensuring that this approach is easily adaptable to your own projects and software needs.

[Fastmixture](https://www.biorxiv.org/content/10.1101/2024.07.08.602454v1) is a software tool designed to estimate ancestry proportions in unrelated individuals. It analyses genetic data to determine the percentage of various ancestral backgrounds present in a given individual. This tool is essential for understanding demographic histories and modeling population structure. You can view the results of running such analyses in the figure below.
...
1. Pull `fastmixture` image from [DockerHUb](https://hub.docker.com/r/albarema/fastmixture). Software GitHub account [here](https://github.com/Rosemeis/fastmixture). 
2. Download and unzip the toy data 
3. What is the command you need to run container? Do not forget to mount the data (use the flag `-v /path/toy:/path/mnt`). Before executing the software, verify that the data has been correctly mounted (e.g. running the `ls` command inside the container)
4. Run fastmixture software, the command to use is: `fastmixture --bfile <input.prefix> --K 3 --threads 4 --seed 1 --out <output.prefix>`. We will set K to 3 because there are three populations (clusters) in our PCA analysis (exploratory analysis). Both `--bfile` and `--out`require the prefix of a filename, so do not include the file extension. If you have checked the toy folder, you will find the files named `toy.data.*`; therefore, use `--bfile toy.data`.
...
- **Apptainer** on HPC / local machine: on your local machine you will need to modify lima.yml to make the current directory (pwd) writable. Alternative, write the data out to /tmp/lima! 
...
:::{.callout-tip collapse="true"}
# Bonus 2: Building a docker image and run your own container 
...
3. Testing the custom image. Let's verify if the custom image functions as expected, running the following command: 
