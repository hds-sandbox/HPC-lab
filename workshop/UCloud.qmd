...
1. Name and version of the app to run (`Coder - 1.92.2`)
2. Job settings: enter a job name (descriptive of the task), select the time (in hours) we want to use a node for (it can be modified afterwards!) and the machine type (selecting a 1 CPU standard node with 6GB memory)
3. Add folders to access while in this job (select your own drive!)
  - You need our access to our shared folder so that you can get the correct software environment
...
4. Choose the file `shared/HPCLab_workshop/setup.sh`. The initialization file,`setup.sh`, is located on a different drive than your own.
...
- Supported version control integration  and formatters tools​
