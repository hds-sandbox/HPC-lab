...
- `shared/hpclab-workshop`: contains input files and scripts. You can read-only access from this directory (no write permissions). Here are the paths you will need during the exercises:
    - `/work/HPCLab_workshop/scripts/`: path to scripts 
    - `/work/HPCLab_workshop/pipes/data`: path to plink simulated data
...
Finally, navigate to your WD, `pipesOut`, and to create a new subdirectory `fastmixture` and save here all files from this exercise (Snakefile, config.yml, output file, etc.).
