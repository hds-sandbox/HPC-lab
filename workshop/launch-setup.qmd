...
Several commands in this section require elevated privileges on Windows. Right-click **PowerShell** and select **Run as Administrator** before you start, otherwise you may encounter cryptic permission errors (e.g. `ssh-agent` failing to start).

*If you are using a different terminal application, ensure you run it as an administrator.*
:::
...
We will specify the type of key with `-t` and save it under a descriptive filename (e.g. `id_UCloud`). Don't enter a passphrase (for now!).

The commands below generate the key pair directly without any interactive prompts:

```{.bash filename="Mac/Linux"}
ssh-keygen -t ed25519 -f ~/.ssh/id_UCloud
```
&#32;
```{.bash filename="Windows (PowerShell)"}
ssh-keygen -t ed25519 -f "$env:USERPROFILE\.ssh\id_UCloud"
```

:::{.callout-important collapse="true"}
# Windows users: you might need to run a couple more commands!
...
# Alternative: interactive approach (you will be prompted for a filename)
```{.bash}
ssh-keygen -t ed25519
```
```
...
...
```
When prompted **Enter file in which to save the key**, type `id_UCloud` (do not just press Enter — that keeps the default name and will cause the later steps to fail).
:::

#### 1.2. Add private key to `ssh-agent`

Once you have generated your SSH keys, add the private key to your system's `ssh-agent` (`id_UCloud` is the filename you specified when generating the key):

```{.bash filename="Mac/Linux"}
ssh-add ~/.ssh/id_UCloud
```
&#32;
```{.bash filename="Windows (PowerShell)"}
ssh-add "$env:USERPROFILE\.ssh\id_UCloud"
```
...
```{.bash filename="Mac"}
cat ~/.ssh/id_UCloud.pub | pbcopy
```
&#32;
```{.bash filename="Linux"}
cat ~/.ssh/id_UCloud.pub | xclip -selection clipboard
```
&#32;
```{.bash filename="Windows (PowerShell)"}
cat "$env:USERPROFILE\.ssh\id_UCloud.pub" | clip
```

You can now paste the public SSH key on UCloud. Go to [**UCloud → SSH Keys → Create**](https://cloud.sdu.dk/app/ssh-keys/create) and paste the key there.
...
![](./images/terminalResources.png){width="750" height="300"}
...
![](./images/addFolder.png){width="750" height="225"}

6. Scroll down and remember to click on  *Enable SSH access*. 
![](./images/enableSSH.png){width="750" height="225"}

Now you are ready to click on the submit button (and wait!). 

Once the job starts, click the **SSH** tab in the progress view to reveal the SSH command, then copy it and paste it in your local terminal:
![](./images/sshCommand.png)

For example: `ssh ucloud@ssh.cloud.sdu.dk -p 2126` 

## 2. Working with files on HPC

### FS navigation and text editors
In this exercise, you will practice working with files and directories on UCloud using the command line. Go back to your job on UCloud and click on 'Open Interface', or run the exercises through the SSH session. 
...
1. What is the working directory (wd) on UCloud? Type only the top-level directory (the root level) `r fitb(c('/work', 'work/', '/work/', '/home/ucloud', '/home/ucloud/'))`
2. Make `hpcLaunch` your wd. Use `pwd` command to print the current wd and copy/paste it here `r fitb(c('/work/hpcLaunch', '/work/hpcLaunch/', '/home/ucloud/hpcLaunch', '/home/ucloud/hpcLaunch/'))`
3. Are there any files in your working directory? `r torf(FALSE)`
4. Create a new directory named `day1` Type the full command `r fitb(c('mkdir day1', 'mkdir -p day1'))`
5. Create a new file called `hello.txt` containing the text *Hello* in the **day1** directory. 
6. Create an empty file named `emptyFile.csv` in the **day1** directory? Type *only* the unix command to do this operation `r fitb(answer='touch')`
...
10. Use a command to list **all** files and their sizes in this directory. Type the command `r fitb(c('ls -lh', 'ls -lh .','du -sh *'))`
...
12. Print the last command in `greetings.txt` using `tail`. Type the command `r fitb(c('tail -n1 greetings.txt', 'tail -1 greetings.txt'),ignore_ws = TRUE)`
13. Can you remove all *.txt* files (specify the file extension when deleting)? Type the command `r fitb(answer='rm *.txt')`

Need some help? See the hints and solutions below. When you are done with the exercise, cancel the job by holding the `Stop application` button. 
