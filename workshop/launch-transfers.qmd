...
An archive (incremental) copy can be done with `-a` option. You can add a progress bar durint the transfer with `-P` option. In this exercse, we want to exclude some files from the transfer: we want to keep only those with `fastq` extension. Run the following command:
...
While inside `less`, type `/N`. Is some text highlighted?

Finally, count how many lines contain at least one N in `file1.fastq`, use the command `grep`. 
...
`tmux` was originally desgiend as a keyboard-only software. However, you can also configure it to allow switching between windows and panes using the mouse. To enable this, add the following setting to the configuration file:
...
4. Change between windows wirth `Ctrl+b` then `n`
...
You are done for now! It's time to stop the job, by holding the `Stop aplication` button to do so. 
